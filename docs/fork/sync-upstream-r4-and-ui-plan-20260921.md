...
> 操作层见配套施工手册 `docs/fork/sync-upstream-r4-runbook-20260921.md`（43 笔枚举清单＋席位任务卡＋命令模板＋重启操作单＋台账格式）。
...
4. `b6fb29850`（31 文件）按上游序早位落（勿挪末位——会喂冲突给后续 TUI 笔），独立 commit，冲突面失控→整笔弃＋下游干跑重算。
