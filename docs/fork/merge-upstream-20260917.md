...
**#2098 量化与三刀方案（08 号件）**：单次 refinement apply 今天作废 **47,330 B = 76.1%** 提示前缀（harness digest 11,365 B 夹在 47KB 高稳定内容前），静态化后作废 0 B、提示 62,181→50,816 B、另省 30-35ms/次读盘；三刀＝核心静态化 / 自有五重建点收口+SC-2 替代信道（前置）/ 压缩恢复面；R1 双投递裁为「保留 outcome 可见、notice 裁剪去重」；排期在 P2 ② 且先于 #2241 二期。**验收前置（R3）**：立项前先落一组背靠背 provider 读数（同会话同 prompt，harness 状态清空 vs 满载，读 cache_read/cache_creation input tokens），否则 L 档投入无法验收。
