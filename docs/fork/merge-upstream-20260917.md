...
- **§4.5 五组兼容测试落地形态**：`bae391973` 一次加了 5 个用例跨 5 个文件——①能力位门（`sends abort_and_send_queued only behind the advertised daemon capability`）、②合并 revision 上的 capability+schema 双门、③跳号前身份拒绝（`refuses abort_and_send_queued on the pre-jump fork schema revision`）、④未知 errorInfo code 降级（`daemon-errors` 新 describe：未知 code 丢 payload 而不误判型 + 本线真有的 code 仍走 typed 路）、⑤busy stale daemon 按跳号前身份拒绝替换。P5 预准备的 busy-stale 拒绝组已实测正控过、负控红。**第 ③ 组「服务端强制声明面正控」已核实存在并可红（2026-09-18 母席亲验，原"缺口"观察撤销）**：当初只在 `bae391973` 那笔的 diff 范围内扫描，故记为未见单列；全仓核后它落在 `test/daemon-protocol.test.ts` —— `DAEMON_DEFAULT_SERVER_CAPABILITIES` 含 `abort_and_send_queued`（:600）、first-party 可声明面（:603）、`DAEMON_COMMAND_PLANE`＝session（:604）、以及 `missingDeclaredCommandCapability` 的**正反两腿**（:852-869：声明了⇒放行、未声明⇒拒），对应 pin 名 `server-enforces capability-gated commands and control-plane authority`。**变异实测（scratch worktree，不在主仓工作树上做）**：从 `DAEMON_DEFAULT_SERVER_CAPABILITIES` 删掉该命令 ⇒ `daemon-protocol.test.ts` **33 例中 3 红**（`capability- and schema-gates abort-and-send-queued at it…`／`server-enforces capability-gated commands and control-pl…`／`keeps every command capability declarable by a first-par…`），复位后 worktree 已 remove+prune、主仓 tracked 面零改动。⇒ §4.5 五组兼容测试**全绿且各组均可红**：`daemon-protocol.test.ts` 33/33（母席复跑）。
...
| ~~#2241 二期~~（相关性排序 digest / query 排序） | **已落**（`05c8f2972`/`a5f4868c0` 排序单次打分＋字节存命中＋1/16/17/48/58 词等价针；`7baa7e5ab` Python IDF；`4adab8a77` G5 接线；`81f964022`/`3c862d9fb` G6 指纹＋针；`2d79973ab`/`8402f5618` 零分窗回落 recency 且不打 ranked 标） | 一期残件已补两笔待应用：`950f72a31`（refinement 排序窗 IDF 加权）/`ba5e8465b`（Python `harness.search` 按文档频次折扣），产物 `/tmp/p2241/patches/`（含 APPLY.md）；**G5 接线与 G6 指纹机制在做**；#2098 已落地 ⇒ 二期排序的前缀缓存风险已解除（原「命中率 94.3%→≈0、账单 $3,019→$14,976」不再是拦路石） |
| **#2334 调和** | **已落地**（v2 十四笔，主仓 `96af873b2` 末端，已推） | 上游 `dca77ecfe` 全量移植，排序＝**lane(QP-4 第一轴) > priority(lane 内) > 到达序**；`selectFirst()`/`canSelectSessionAction()`/压缩闸判据/`input-classification.ts`/`moveQueued`/协议 rev38 一字未动；判据从 fork 单一分类点派生（`sessionActionPriorityForInputClass`＝origin==="human"?"user":"background"，＋`isAgentSessionMessageId` 结构守卫），副作用红利＝#2098 的 harness_digest 分类行自动落 background。v1 五笔（`5ea998daf`/`20942fa2d`/`7efaf9352`/`d6d29417d`/`3b3d8808c`，rebase 零冲突）＋v2 九笔（F1 `fdbfa1cc1` 抄上游 4 行并补钉 idle cron 路径／F6+F7 `9a100d3b5` 清 5 处 `as never`、6158 改用 `conversationMessages()`、删事实错误注释／F3 `d49b16669` 手排钉改**跨 rank**（store `moveQueued` 腿＋新增 `swapQueued` 腿＋session 级走 `mutateQueuedMessage` 真实入口）／F4 `febd8c2ab` goal pinned **经生产路径**的 session 级钉（观测面＝恢复快照，因 goal context 非 queue-visible）／F2 `157f4ca66` 据实注释／F10 `1661b777d` 三处文档补 extension 半句＋README 补 goal-context 前插这唯一例外／F5+F8 `171281de7`/`0bf472576`/`abc8638a6` 头注改实测口径＋引用改仓内路径＋DESIGN/APPLY 落 `docs/fork/designs/`）。**异构复核判 PASS 可应用**（K3 席复验 P1-P14：77 文件 runset base 1075 例/after 1094 例 **0 fail**、daemon-mode **198/198**、tsgo/biome/hygiene 全 0、弱化扫描 0 命中、`apply --check` 与 cherry-pick 到 `b2e1dc54c` 双干净、文档笔无夹带）；**三条不互撕全部变异可红**（M6=2红/M6b=1红/M7=1红/F1b=3红+1红/M1=11红含老板现场例/C7=1红），M4=0红＝闸的 human 半支**不可达**（四环证据），已据实注释并记设计债。**母席亲跑落地闸**：tsgo 0、biome 1494 文件 0、448 例（daemon-mode 198/store 44/goal 52/2098 digest 19/2334 7/6158 4/recursion 124）0 fail、181 例（queue 113/matrix 20〔设计债修复后为 **22**〕/child-reply 8/compaction 40）0 fail、`check:process-smoke` GATE GREEN。**闸口径教训（登记）**：邻接集必须「关键词邻接集 ∪ 补丁波及测试」——v1 自报"75 文件 843 例 0 fail"不含 daemon-mode，只跑 73 会全绿放行、照不出那 3 红。**残余设计债＝已闭环**（`r2-debt` 席五笔 `ede1f8da8`/`08dd5c00f`/`02b0a6bdb`/`098bd2584`/`d7e390def`，已推）：①**hung 压缩时排队输入无 stall 上界**——base 已复现（压缩在队列空时由 fill turn 的 agent_end 阈值启动 ⇒ `hasPendingSessionWork` 武装无可武装，之后 human prompt 进来 isCompacting 20s 不变、prompt 永不落地、其余 pin 全绿），且**洞比原口径更宽**：同窗口的 heartbeat（scheduled/background）一样永挂。修法一行落在唯一入口 `_admitSessionInput`：`if (this.isCompacting) this._armCompactionGateWatchdog();`——**不按 priority 过滤**（只按 user 过滤会留下已复现的 heartbeat 红，且要为此另立第二套启发式）；不变式＝「只要有输入在压缩后面等，这个压缩就有上界」，与 `hasPendingSessionWork` 那处对称。效果红线自查：不排序、不延迟、不吞输入（human 反而更早落地）；预算仍 `stallWatchdog.abortAfterSeconds`，出厂 0 回退 `COMPACTION_GATE_ABORT_AFTER_SECONDS`(600s)；无 `_compactionOperation` 时 no-op（branch summary 计入 isCompacting 也不会被误切）；QP-4 与 #2334 的 lane>priority>到达序未动。新钉两行（matrix 20→**22**，基线 20 例断言一字未改）；**变异实测**：M1 撤修⇒2 红（human 行+machine 行）、M2 收窄成 `priority==="user"`⇒1 红（只红 machine 行，即偏离①的机械依据）、M3 掏空 `abortCompaction()`⇒3 红（含既有 watchdog 行 ⇒ 钉量的是「压缩真被切」不是「定时器真被挂」）；母席独立复跑 M1 亦 2 红、复位后 `git diff` 空。②**压缩闸 human/fence 半支仍不可达**（走"注释保留+说明为何仍不可达"分支）：四环自复现，其中 ring4 **订正了原措辞**——`incomingInputFactsFromMessage()` 其实**有** `context.isSystemFence` 入口（显式给 true 能分类出 `system_fence`＝正控腿），但闸那处调用的 context 字面量只给 `{source, agentMessageId, streamingBehavior}` ⇒ 五种 source 下 `facts.isSystemFence` 全 undefined；结论不变、注释已改准。M0 实测（删 stand-down 行）matrix(22)+2334(7)=**29 passed/0 failed** ⇒ 仍不可观测（注释计数 27→29 并写明拆分口径）。本次修法只加了一次武装调用、未搬裁决、调用点集合仍为 1 ⇒ 半支仍不可达，注释明写「这不是所警告的那次搬迁」。同窗口 `agentmsg_*` 正控腿已存在（`2334-human-priority-lane-scoped.test.ts`：child reply⇒`compaction_pending`、human⇒`undefined`、放闸后 human 先落地），human 行注释指向它、未在 matrix 重复造。 |
...
| §4.5 第 ③ 组「服务端强制声明面正控」 | **已核实存在且可红（观察撤销）** | 全仓核后它在 `test/daemon-protocol.test.ts`：`DAEMON_DEFAULT_SERVER_CAPABILITIES` 含该命令（:600）、first-party 可声明面（:603）、command plane=session（:604）、`missingDeclaredCommandCapability` 正反两腿（:852-869），pin 名 `server-enforces capability-gated commands and control-plane authority`。**变异实测**（scratch worktree `/tmp/wt-cap`，非主仓工作树）：从默认服务端能力集删掉该命令 ⇒ 33 例中 **3 红**；复位后 worktree remove+prune、主仓 tracked 面零改动。原观察的成因＝扫描面只取 `bae391973` 那笔的 diff |
...
## 9.3 收口窗口在飞与 backlog（2026-09-18 晚记）

**在飞（克隆施工、母席逐系列 cherry-pick + 闸 + 推送）**：协议面接线与两句文档改真（fix-protocol：B3-01 Esc 发排队、B3-04 响亮退化、B3-08 priority 回填针、B3-09' 枚举派生、FA-1 #2130 补落、FA-6/FA-9）；压缩/daemon 面（fix-compaction-final：B2-C01 看门狗 abort 自解挂泵＋针去 hand-wake、B2-15 预算针、B2-16/17、B2-C02、B2-C06/07/09、FA-8、perfB②③）；perf 面剩余（fix-perf-final：一拍一次 getContextTree、FA-5 顶栏认应急开关、B1-02 缓存字节上界/LRU/TTL）；TUI 展开有界预算＋show-all＋缓存＋R8/R10/R11（fix-tui-expand，配对复量 14×、show-all 回到旧行为＝没砍功能）；压缩算法面（perf-shrink：planEmergencyShrink 后缀和线性化、buildCompactionAppendix 单遍、同 turn 密度扫描 memo）。

**backlog（收口冻结后另开一轮，不挡 0.10.0）**：R9 visibleWidth 冷 miss＋widthCache 打穿（硬门＝equivalence-check 65536 码点穷举）与 resize ~80ms 尾随防抖（最后一次必画、stop/dispose 清 timer）同席专做；kernel venv 冷建 20.6s 串行 import 探针与暖 boot 443ms 同类探针；内核 bash 每次同步 fork /bin/ps（host 侧已异步、kernel 侧反向，修法需异构复核 fail-closed）；压缩头 digest 无条件渲染（perfC 判接受，再省需 stamp 门复用＋异构复核）；ledger canonical memo；owner 断连 30s 宽限的行为面裁量（归因日志＋针在飞，行为是否放行为设计待裁）；`footer-data-provider.test.ts` 的 waitFor 超时在 loadavg>20 不够用（红名会漂移）列入负载敏感清单或做成可注入；跨语言同序针（TS digest 窗 ↔ Python harness.search，六条故意分歧已登记，只钉不改平手规则）。

**读数纪律（本轮立）**：负载档读数必须核 ppid 归属（宽 grep 会把别席负载生成器算成自己的孤儿）；同窗口成对比值优先、绝对 ms 需安静窗（vm.loadavg < 核数一半）复测；未被触及的负控 driver 标定噪声底（本机 loadavg 20-46 下约 ±20-50%，<1.5× 单批读数不作结论）；锚必须落在被核对的载体里、不采信自报。
