...
export function createRetryCapFetch(
	...
	const hasSink =
		options.onProviderRetry !== undefined ||
		options.onProviderRequestAttempt !== undefined ||
		budget !== undefined;
	...
	return async (input, init) => {
		...
		if (delayMs !== undefined) {
			const retryAfter =
				response.headers.get("retry-after") ?? response.headers.get("retry-after-ms") ?? undefined;
			try {
