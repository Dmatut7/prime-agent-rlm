...
describe("openai-completions tool_choice", () => {
	...
	it("sends an explicit tool_stream false even when the zai shorthand is on", async () => {
		const baseModel = getZaiTestModel();
		const model = {
			...baseModel,
			compat: {
		...
		const params = (payload ?? mockState.lastParams) as { tool_stream?: boolean };
		expect(params.tool_stream).toBe(false);
	});

	it("sends an explicit tool_stream true from compat alone", async () => {
		const baseModel = getZaiTestModel();
		const model = {
			...baseModel,
			compat: {
