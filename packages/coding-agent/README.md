...
Who overtakes whom: the two queues are not FIFO relative to each other - the steering queue always drains before the follow-up queue, even when a follow-up was queued earlier. Within a single queue, input you send goes ahead of queued subagent replies and other machine traffic (scheduled prompts, heartbeats, harness notices), and arrival order is preserved among messages of the same kind; reordering a queued message by hand is never undone by a later arrival - a later arrival picks an insertion point, and the only thing that takes the head of a queue ahead of your own ordering is a goal context, which front-inserts without re-sorting what you arranged. A user message an extension submits on your behalf is ruled human by the same structural test the compaction gate uses, so it shares your priority and the two rulings never disagree; that test reads the message's source and shape rather than who typed it, so an extension is not mistaken for you - it is just not filed below machine traffic either. Subagent replies and heartbeat prompts are delivered as steering by default, so they overtake follow-ups you queued first. A compaction in flight still outranks a queued subagent reply, which waits for it instead of interrupting it; your own input is never held by that gate, and once the compaction settles the queue drains your input first. While admission is paused (an MCP server reload, an ACP stop window, or update-restart teardown), new input is refused with a retryable error instead of being accepted and then lost; during restart teardown the parked queue survives into the restart instead of running mid-shutdown.
...
Daemon identity is the agent directory, and a client reaches its daemon through the supervisor registry wherever that daemon chose to listen, so `shutdown` and `doctor --fix` scope to this process's own services by default: the socket directory the shell resolves (`$TMPDIR/prime-agent-<uid>`) plus the sockets of the live daemons that own this agent directory. Both print a named report of every affected socket, its pid and its live session count before anything is stopped, and `--force` prints that report too instead of skipping it. `--all` widens to every daemon discovered on this machine, `--socket <path>` / `--socket-dir <dir>` narrow to one service or one directory, `--dry-run` prints the plan without acting, and `--orphans` refuses anything that still has live sessions, worker processes or cpu. `--force` additionally kills unresponsive workers, their tracked child processes and the hidden supervisors left behind on a socket that is already in scope, but it never widens the scope: another agent directory's live services stay running unless this shell can talk to them too.
...
| `DO_NOT_TRACK` | Opt out of every request the agent starts on its own - aggregate usage analytics, the startup release check and automatic trace sharing - when set to `1`/`true`/`yes` |
