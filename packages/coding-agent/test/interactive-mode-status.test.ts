...
describe("InteractiveMode pending bash components", () => {
	...
	test("does not double-prefix labeled injected queue previews", () => {
		...
		expect(rendered).toContain("定时任务：check steering");
		expect(rendered).not.toContain("插话：定时任务");
		expect(rendered).toContain("目标：steer goal");
		expect(rendered).not.toContain("插话：目标");
		expect(rendered).toContain("插话：plain steering");
		expect(rendered).toContain("定时任务：check status");
		expect(rendered).not.toContain("稍后发送：定时任务");
		expect(rendered).toContain("目标：continue goal");
		expect(rendered).not.toContain("稍后发送：目标");
