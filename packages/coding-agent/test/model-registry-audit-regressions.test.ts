...
describe("ModelRegistry audit regressions", () => {
	...
	describe("getExecutableModels stale-cache fallback", () => {
		...
		test("transient catalog failure keeps codex models from the stale cache", async () => {
			...
			const kept = codexModels[0]!;
			const dropped = codexModels[1]!;
			...
			globalThis.fetch = (async () => {
				...
				return new Response(JSON.stringify({ models: [{ slug: kept.id }] }), {
					status: 200,
			...
			try {
				const first = await registry.getExecutableModels();
				expect(fetchCount).toBe(1);
				expect(first.filter((model) => model.provider === "openai-codex").map((model) => model.id)).toEqual([
					kept.id,
				]);

				// Age the cache past its TTL so the next call must refetch, then fail the refetch.
				failFetch = true;
				vi.setSystemTime(Date.now() + 301_000);

				const second = await registry.getExecutableModels();
				expect(fetchCount).toBe(2);
				// Pre-fix, the catch block re-ran the freshness guard that had just failed above,
				// so the fallback was dead code and every codex model was dropped.
				expect(second.filter((model) => model.provider === "openai-codex").map((model) => model.id)).toEqual([
					kept.id,
				]);
				expect(second.some((model) => model.id === dropped.id)).toBe(false);
			} finally {
				vi.useRealTimers();
