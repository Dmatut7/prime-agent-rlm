...
describe("retention sweep - stale leases (red test R-10)", () => {
	function leaseFixture(f: Fixture, name: string, owner: Record<string, unknown>): string {
		...
		writeFileSync(
			...
			JSON.stringify({
				version: 1,
				token: "t",
				// Pin the owner identity explicitly. `isReclaimableOwnLease` compares this field with
				// `PRIME_AGENT_INTERNAL_SESSION_LEASE_OWNER_ID`, so a record that omits it reads as a
				// leaked lease of the *test process* whenever that env var is absent (the sanitized-env
				// convention), and the live-pid fixture below would then be reclaimed.
				activeSessionId: "retention-fixture-owner",
	...
	it("does not delete a dead owner's lease during a dry run", async () => {
		const f = fixture();
		const dead = leaseFixture(f, "e5", { pid: 999_999 });
		const report = await runRetentionSweep({
		...
		expect(report.dryRun).toBe(true);
		// The prediction is the same judgement; only the removal step is skipped, so the
		// directory is still there and the class still reports what it would have reclaimed.
		expect(existsSync(dead)).toBe(true);
		expect(classResult(report, "stale-leases").reclaimed).toBe(1);
	});
...
describe("retention sweep - circuit breaker and read-only dry run (review N-1/N-2)", () => {
	...
	it("does not unlink a stale venv reference during a dry run", async () => {
		// The venv class reports stale references while it reads a generation, so the read
		// itself must be read-only for a dry run (review N-2). Without the guard the file is
		// gone after a `--dry-run`, which is a filesystem change a dry run promised not to make.
		const f = fixture();
		const base = join(f.root, "kernel-venv");
		mkdirSync(base, { recursive: true });
		const generation = `${base}-cccccccccccc`;
		mkdirSync(join(generation, ".in-use"), { recursive: true });
		writeFileSync(join(generation, "payload.bin"), "payload");
		// Reference files are named after the pid with no extension (venv-in-use.ts).
		const staleReference = join(generation, ".in-use", "999999");
		writeFileSync(staleReference, JSON.stringify({ version: 1, pid: 999_999 }));
