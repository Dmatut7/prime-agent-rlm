...
/** See the fork adaptation note in the denied-response case below. */
const DENIED_FAILURE_KINDS = new Set<unknown>(["unknown", "permission"]);
...
registerHooks({
	...
	it.each([
		...
		if (outcome === "success") {
			...
			// Fork adaptation: upstream 8a1e95800 (#2045, ADAPT-level, not taken here)
			// classifies a 403 AccessDeniedException as "permission"; without it this
			// fork reports "unknown". Both are accepted so the case keeps pinning what
			// this commit fixes - the bundled CLI loads Bedrock and the failure reaches
			// the shared JSONL log with its structured fields.
			expect(DENIED_FAILURE_KINDS.has(failures[0]!.kind)).toBe(true);
			const diagnostic = messages[0].diagnostics?.find((entry) => entry.type === "provider_stream_failure");
			expect(DENIED_FAILURE_KINDS.has(diagnostic?.details?.kind)).toBe(true);
