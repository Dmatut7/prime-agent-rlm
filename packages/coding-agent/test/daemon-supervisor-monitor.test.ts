...
describe("daemon worker supervisor monitoring", () => {
	...
	it("preserves cached summaries when recovery omits the assigned root", async () => {
		...
		// The non-recovery leg reaches the reconstructor loop, so the fake needs one.
		const supervisor = Object.assign(Object.create(DaemonSupervisor.prototype), {
			streamReconstructor: { seed: vi.fn(), clear: vi.fn(), hasPartial: () => false },
		}) as {
		...
		// The retired options bag read as a truthy recovery whatever it held, so
		// the negative leg is what pins the second positional as `recovery`.
		await expect(supervisor.refreshWorkerSummaries(worker, false)).resolves.toBeUndefined();
	});
