...
`,
	...
	it("exits an orphaned session worker when no replacement supervisor can come up", { timeout: 120_000 }, async () => {
		...
		const agentDir = join(root, "agent");
		const projectDir = join(root, "project");
		// spawnSupervisor isolates the registry root here; the test reads the
		// supervisor's owner record from it.
		const registryDir = join(agentDir, "supervisor-registry");
		// The socket path must stay short: macOS rejects listen() with EINVAL past
		// the sun_path limit, and the per-test temp dir nests too deep.
		...
		// A 3s supervisor-lost window keeps the orphan exit inside the test budget.
		const supervisor = spawnSupervisor(agentDir, socketPath, projectDir, [], {
		...
		// SIGKILL the registry-recorded supervisor pid: the spawned child can be a
		// tsx wrapper, and killing the wrapper alone leaves the supervisor listening
		// on its socket, so the worker would never orphan.
		...
		// A read-only socket directory makes every replacement-launch attempt fail
		// (lock/bind EACCES): the real-world orphan precondition where a worker
		// cannot bring its supervisor back.
