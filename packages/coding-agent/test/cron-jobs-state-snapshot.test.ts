...
describe("AgentCronJobStore state snapshots", () => {
	...
	it("serves read-only views: an in-place edit of a listed job cannot change store behavior or reach disk", () => {
		...
		// The parse path is frozen as well as the published one: an external writer's
		// replacement invalidates the snapshot, so the next read comes from disk and
		// must not hand out a mutable graph either.
		writeExternalState(
			storePath,
			[
				externalJob({
					id: "external-view",
					prompt: "external copy",
					sessionId: "session-1",
					sessionFile: "/tmp/session-1.jsonl",
				}),
			],
		...
		const externalListed = store.list();
		expect(externalListed.map((candidate) => candidate.id)).toEqual(["external-view"]);
		expect(Object.isFrozen(externalListed[0])).toBe(true);
		expect(() => setStatus(externalListed[0], "cancelled")).toThrow(TypeError);
		expect(store.list()[0]?.status).toBe("active");
	});
	...
	it("re-reads instead of caching an external replacement that lands right after our own write", () => {
		...
		// This fork's create reads the write back and fails loudly when the merge kept
		// another writer's copy, so the raced write is reported as dropped rather than
		// silently returning a job that never reached disk. The snapshot property under
		// test is the same: the read-back came from the disk, not from our own write.
		expect(() =>
			store.create({
				activeSessionId: "active-2",
				sessionId: "session-2",
		...
		).toThrow(CronJobsWriteDroppedError);
