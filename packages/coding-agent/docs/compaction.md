...
- The estimate priced text at chars/4, which under-counts CJK: a Chinese-heavy session measured 1.60x low, and CJK costs about one token per 1.5 characters rather than one per four. The estimate that feeds the trigger is now priced by content density (see the caliber table in [`content-density.ts`](../src/core/compaction/content-density.ts)), so what the 80% ratio still holds back is the second error below plus tokenizer drift.
...
Two token calibers exist ([`compaction.ts`](../src/core/compaction/compaction.ts), [`content-density.ts`](../src/core/compaction/content-density.ts)), and every multiplier they quote is listed in one table in `content-density.ts`:

- `estimateTokensByContent` prices by content density - CJK at 1.5 chars/token, fenced code at 3 chars/token, ASCII at 4 chars/token - so the correction it can apply is bounded at 2.67x on a fully-CJK text and is exactly 1.00x on ASCII prose. It is the caliber of everything that decides how much context is retained or reported: the trigger, `/usage`, the cut point, `keepRecentTokens` and the emergency shrink. The trigger and `/usage` need it because both are compared against a limit the provider measured; the cut point and `keepRecentTokens` need it so a nominal retained token means an estimated real token instead of a chars/4 one that reads up to 2.67x low. A recorded provider measurement of one whole Chinese-heavy session sits at 1.60x, below the per-class bound.
