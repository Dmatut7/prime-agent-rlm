...
Who overtakes whom: the steering queue always drains before the follow-up queue, even when a follow-up was queued earlier; within one queue input you send goes ahead of queued subagent replies and other machine traffic (scheduled prompts, heartbeats, harness notices), and arrival order is preserved among messages of the same kind. A user message an extension submits on your behalf is ruled human by the same structural test the compaction gate uses, so it shares your priority and the two rulings never disagree; the test reads the message's source and shape rather than who typed it, so an extension is not mistaken for you. Subagent replies and heartbeat prompts are delivered as steering by default, so they overtake follow-ups that were queued first. A compaction in flight still outranks a queued subagent reply - the reply waits for the compaction instead of interrupting it - while your own input is never held by that gate and drains first once the compaction settles. While admission is paused (MCP server reload, ACP stop window, update-restart teardown), new input is refused with a retryable error - nothing is accepted and then lost - and during restart teardown the parked queue is handed to the restarted session instead of running mid-shutdown.
