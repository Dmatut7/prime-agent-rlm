...
- `customInstructions`: The custom compaction instructions this summary was built from, if any
- `usage`: Token `Usage` of the summarization request itself
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by Prime Agent (legacy field name)
- `usage`: Token `Usage` of the summary request itself
...
Records where the session was left by an explicit position change (`branch()`, `resetLeaf()`). Resume resolves the active leaf from the last line of the session file, so without this marker a leaf that only lives in process memory would be silently undone by a restart. The marker describes the leaf position as metadata: it hangs off the entry it points at (`parentId` is that entry), and it never reaches the model.

```json
{"type":"leaf_position","id":"h8i9j0k1","parentId":"a1b2c3d4","timestamp":"2024-12-03T14:20:00.000Z","targetId":"a1b2c3d4"}
```

Fields:
- `targetId`: Entry the session moved to, or `null` when the leaf was cleared (rewound before any entries)
...
- `allowsPersistence()` - Whether the session writes to disk (in-memory sessions return false)
