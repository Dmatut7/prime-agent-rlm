...
| Setting | Type | Default | Description |
|---------|------|---------|-------------|
| `idleEvictionMinutes` | number or `"off"` | `90` | Idle threshold in minutes for whole-tree worker eviction (and the cap on `childIdleEvictionMinutes`); `"off"` disables all idle eviction, children included. A session whose kernel is executing a cell or still owns a live `bash()` handle is not idle, however old its last message is. |
| `childIdleEvictionMinutes` | number or `"off"` | `20` | Idle threshold in minutes for closing an individual subagent session while its parent stays open. The transcript is kept and a message wakes it, but its Python kernel state is not. Never longer than `idleEvictionMinutes`; `"off"` uses `idleEvictionMinutes`. |
...
`idleEvictionMinutes` and `childIdleEvictionMinutes` are global daemon policies and are read only from `~/.prime/agent/settings.json`. Set it to a positive number to configure the idle threshold. The `daemon.*` supervisor-policy keys default to the shipped behaviour, so an absent `daemon` section changes nothing.
