...
export async function catchUpAgentTraceUploads(
	...
	const beforeRequest = createTraceUploadAllRequestGate(options.signal, options.onUploadDelay);
...
class AgentTraceUploadController {
	...
	private start(): void {
		if (this.running) {
			// Keep the work pending so the running leg's settle path re-arms: the coalesce notice
			// always has a guaranteed follow-up behind it.
			this.pending = true;
	...
	/**
	 * Settle everything this controller would still write: the running leg, and an armed debounce
	 * that has come due.
	 *
	 * A debounced upload is still work in progress, and waiting only on the running leg lets a
	 * caller tear down during the delay and drop it. A due arm is therefore flushed rather than
	 * waited for, so draining never depends on wall-clock time. Bounded, because a retry re-arms.
	 */
	async whenIdle(): Promise<void> {
		for (let drain = 0; drain < MAX_IDLE_DRAIN_CYCLES; drain += 1) {
			...
			if (this.timeout !== undefined) {
				if (!this.isUploadDue()) {
					// Backoff outlives this drain: the durable outbox replays it on the next
					// catch-up rather than overrunning the server's Retry-After.
					return;
				}
				clearTimeout(this.timeout);
				this.timeout = undefined;
	...
	private async runScheduledUpload(): Promise<void> {
		...
		const upload = uploadAgentTraceFileWithRequestGate(
			{
				...this.options,
...
export function installAgentTraceUpload(
	...
	// The shared promise, not the module-level one: a second install must await the catch-up that
	// is already running, and it must still await it if this module is asked again later.
