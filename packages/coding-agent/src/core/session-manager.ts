...
/**
 * Fork merge of upstream #2433's append-only fast path with this fork's lease
 * discipline: the append runs under the same write lease and
 * `repairOwnedSessionFile` gate as `appendOwnedSessionLineAsync`, but uses the
 * no-parse `append*ToExistingFile` fast path first and falls back to a full
 * `SessionManager.open` only when the fast path cannot place the entry.
 */
export async function appendOwnedFastEntryAsync(
	sessionPath: string,
	agentDir: string,
	...
	try {
		const repair = repairOwnedSessionFile(sessionPath);
		if (repair === "unverifiable") {
			// K3P-45: the trailing record could not be verified whole; appending
			// would glue both lines into one unparsable interior line.
			throw new Error(
				`Refusing to append to ${sessionPath}: its trailing record is unterminated and too large to verify, so appending would glue the lines together and hide both`,
			);
		}
		const fastId = fastAppend();
		if (fastId === undefined) {
			fallback(SessionManager.open(sessionPath));
		}
	} finally {
		lease.release();
	}
