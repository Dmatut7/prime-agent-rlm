	...
	const definition: ToolDefinition<typeof bashSchema, BashToolDetails | undefined, BashRenderState> = {
		...
		async execute(
			...
			if (
				...
				!isTruthyEnvValue(
					spawnContext.env[BASH_DESTRUCTIVE_GIT_BYPASS_ENV] ?? process.env[BASH_DESTRUCTIVE_GIT_BYPASS_ENV],
				)
			) {
			...
			const formatOutput = (snapshot: Awaited<ReturnType<typeof finishOutput>>, emptyText = "(no output)") => {
				...
				if (truncation.truncated) {
					...
					// The temp file can be absent - the persistence that creates it can fail - and
					// sending the reader to "Full output: undefined" is a lie it will act on.
