...
export function createAutonomousContinuationMessage(
	...
	return {
		...
		content: [
			{
				type: "text",
				// Bare continuation prompt: same text as the immediate
				// `nextAutonomousContinuation` path (fork pin; upstream's
				// `[autonomous-continuation]` label is rolled back). A held continuation and an
				// immediate one are the same message to the model, so they must not diverge.
