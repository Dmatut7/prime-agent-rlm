 ...
 *
 * Exactly the code points dropped: C0 controls other than \t \n \r,
 * and the three Unicode Format characters that crash string-width. Everything
 * else is kept - including astral characters and lone surrogates, which the
 * previous `Array.from` + `filter` + `join` implementation also kept.
 *
 * Every member is a BMP code point that cannot appear inside a surrogate pair,
 * so matching code units removes exactly the same characters the code-point
 * filter removed. `scripts/perf/equivalence-check.ts` pins that claim against
 * the original implementation over all 65,536 BMP code points plus a fuzz
 * corpus; the reason to prefer the single native pass is that a 200KB tool
 * result used to allocate one array entry per code point on every panel
 * rebuild, and it does that on the Ctrl+O expansion path for all 50 outputs.
 */
const SANITIZE_DROPPED_CODE_UNITS = /[\u0000-\u0008\u000b\u000c\u000e-\u001f\ufff9-\ufffb]/g;

export function sanitizeBinaryOutput(str: string): string {
	return str.replace(SANITIZE_DROPPED_CODE_UNITS, "");
}
