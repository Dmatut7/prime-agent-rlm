...
export class InProcessAgentConnection implements AgentConnection {
	...
	async resumeQueuedWork(): Promise<boolean> {
		return this.session.resumeQueuedWorkFromConnection();
	}
	...
	async startSideQuestion(
		...
		const run = startSideQuestion(
			...
			previousTurns,
			// No module layer wraps a side question, so the provider client is its outermost
			// retry layer and retries per this session's policy.
