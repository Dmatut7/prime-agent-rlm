...
export interface SupervisorLinkOptions {
	...
	/**
	 * Handshake hello budget. Our accounting keeps the 1s the per-call sites
	 * used before the link existed, so a wedged supervisor costs the same.
	 */
	...
	private async ensureConnectedInternal(): Promise<DaemonClientLike> {
		...
		this.connecting ??= (async () => {
			...
			try {
				...
				await client.waitForHello(this.options.helloTimeoutMs ?? 1000);
			} catch (error) {
				cleanup();
				throw error;
			}
