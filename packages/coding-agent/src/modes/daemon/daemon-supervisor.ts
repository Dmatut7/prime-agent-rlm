...
export class DaemonSupervisor {
	...
	/**
	 * Monotonic epoch claimed at each scan start and at each invalidation: only the
	 * newest claim may publish, so an older scan cannot overwrite a newer scan's rows
	 * and a scan that raced a mutation cannot republish stale rows.
	 */
	 ...
	 * scheduled-jobs artifact. Always scans fresh per caller: management decisions, wake
	 * recomputes and the ephemeral-cancel retries they drive must see the latest disk
	 * state. Every completed scan opportunistically refreshes the snapshot that
	 * heartbeats_list serves, but only while it is still the newest scan: an older scan
	...
	/**
	 * Snapshot-served rows for heartbeats_list responses, where per-request freshness
	 * matters less than not queueing: with hundreds of saved sessions, N concurrent
	 * catalog requests used to enqueue N full scans on the ledger's serialized queue and
	 * push the last response past the client's 30s transport deadline. Daemon-owned
	 * mutations drop the snapshot and detach the in-flight shared scan
	 * (broadcastHeartbeatsChanged and the saved-session delete/rename paths); the next
	 * list or the wake recompute armed by the same broadcast rescans, and concurrent cold
	 * lists share one in-flight scan. A served snapshot older than
	 * PASSIVE_SCHEDULED_JOBS_REFRESH_MS refreshes in the background without being dropped,
	 * so lists that arrive during the refresh keep serving bounded-stale rows and external
	 * artifact writes converge without blocking a response.
	 */
	private async catalogPassiveScheduledJobs(): Promise<PassiveScheduledJob[]> {
		...
		if (cached) {
			if (Date.now() - cached.scannedAt >= PASSIVE_SCHEDULED_JOBS_REFRESH_MS && !this.shuttingDown) {
				// Stale-while-revalidate: serve the snapshot now and refresh in the background
				// without dropping it. A failure only logs; the next read retries.
	...
	/**
	 * Claims the publish epoch when the scan actually starts, so callers that join an
	 * existing in-flight scan keep its rows, while a caller that starts after an
	 * invalidation never adopts the detached pre-mutation scan.
	 */
	private startSharedPassiveScheduledJobsScan(): Promise<PassiveScheduledJob[]> {
		...
		const scan = this.scanPassiveScheduledJobs()
			...
			.finally(() => {
				// An invalidation may have detached this promise and let a newer scan take
				// the slot; clear it only while it is still ours.
	...
	/**
	 * Compare-and-swap publish: a scan may store only while it still owns the newest
	 * epoch, so an older scan cannot overwrite a newer snapshot and a scan that raced an
	 * invalidation cannot republish pre-mutation rows.
	 */
	...
	/**
	 * Daemon-owned mutations invalidate: claim the epoch so in-flight scans can no longer
	 * publish, drop the cached snapshot, and detach the shared in-flight promise so a list
	 * that starts after the mutation rescans instead of awaiting pre-mutation rows.
	 */
	...
	private broadcastHeartbeatsChanged(): void {
		// Every daemon-owned scheduled-job mutation and worker residency change lands
