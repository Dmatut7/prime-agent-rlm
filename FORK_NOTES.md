...
- 审查：设计审 A（deepseek-v4-pro）/B（glm-5.3-prime）+ 交叉审 C（qwen3.8-max）三轮，.pipeline/auto-session-name/ 留计划与三份报告；C 的 6 项阻塞核实后 5 项成立已修（撤谱系继承、空草稿闸、legacy 跳过、重名后缀、子代理闸），1 项（报告不存在）实证不成立。测试 27 条新 + 定向回归 145 条绿。
...
## 2026-09-24（深夜）状态栏说实话：备用链接管时显示实际干活的模型

- 事故背景：当晚断网把会话推上备用链，kimi-k3 顶班四十分钟，状态栏却一直显示配置的 qwen3.8-max——老板收到慢回复和英文垃圾输出，看界面却看不出是谁在答，翻会话转录才挖出真相。状态栏只说「你选的模型」，不说「正在答你的模型」。
- 改法：每条助手消息到达时记下实际服务模型；与配置模型不一致时，状态栏模型段后面加警告色「实际:kimi-k3」，一眼看出备用链在顶班；切回主模型标记自动消失。老 daemon 不带这个字段时不受影响（不显示标记）。
- 测试：footer 新增 2 条（不一致显示/一致或缺字段不显示）+ 记录逻辑 1 条（换服务模型才刷缓存），footer 相关 39 条全绿；biome + tsgo 干净。分支 feat/footer-serving-model。

## 2026-09-24（夜）子代理中文名 + 点击看详情

- 老板看着子代理面板一排英文名（research-cloud-…、img-inspect-1）发火：名字要中文，点了要能看详情。两件事一起做：**名字**——模型起名时被教成「用用户的语言起名」（中文用户就写 `name='调研-云厂商'` 这种，短、角色开头、一眼能认）；模型没起名时系统自动起的名以前只认英文字母，中文任务会缩成 worker，现在中文原样保留（英文名不变，老测试不动）。**详情**——鼠标点面板里任何一行（或点标题行），弹一张只读卡片：状态、模型、走了几步、花了多少 token、领了什么任务、最新回复、报错、会话目录；卡片是活的，子代理还在跑时内容自己刷新，删掉了会告诉你「已不在列表里」，Esc 关掉。键盘 Enter 原来的行为不动（还是进会话列表）。新起的名字是中文，已经在跑的英文名不会自动改。基于上游 0.11.12，点击区域、卡片、命名各带测试。
...
## 2026-09-24（下午二）看图路由补全：中途来图自动借看图模型（PR 待合并）

- 你会遇到什么：以前会话模型看不了图（如 glm-5.3-prime）时，运行中途的截图/attach_image 一上来就被技能拦死（「不支持视觉，请换模型」）；就算图进来了，服务商也悄悄丢图、模型瞎答不吭声——当天三起现场（OCR 锁号 ×2、雇视觉子代理烧 8.7M token、PM 被迫人肉切模型）都是这么来的。现在：只要 settings.imageModel 配了能看图的模型，运行中途冒出来的图会自动借看图模型读完、交回会话模型继续干活；没配或配坏才报错，报错直接说改哪个设置。
- 修的是两个缺口，路由底座（0.11.2 的按批路由 + 读完即交回）本来就有：① attach_image 门禁从「当前模型看不了图就拒绝」改成「先问 harness 能不能路由（新只读请求 image_route.info），能就放行」，老版本宿主问不到则退回原提示；② 从没路由过的纯文本轮次中途第一次来图，当场引导出路由（以前这种图必被静默丢弃）。用户亲手发图的报错语义不变。
- 红→绿：faux 套件 +3（中途首图引导 / 无路由不硬切 / blockImages 不路由）、内核桥 +3、会话级 +2、白名单钉新请求类型；回归换模型/回交 12 条全绿；biome + tsgo 干净。真机端到端也过了：glm-5.3-prime 文本会话中途 attach 真图，服务序列 glm-5.3-prime → kimi-k3（真实百炼请求无报错），kimi-k3 逐字读出图中暗号再交回。测试注意：生产 venv 被在跑 daemon 占住时技能替换会延迟，须用 PRIME_AGENT_KERNEL_VENV 隔离目录跑，否则测的是旧技能。
- 分支 feat/image-route-midrun-bootstrap，PR 待合并后随下版装主力机；工单与施工记录见 docs/fork/vision-auto-route-20260924.md。
