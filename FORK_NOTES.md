...
- **验证**：先红后绿（6 个新测试文件在基线树 `0429806b6` 全红、9 条断言失败，其中两条是断言级红：并发峰值 `expected 1 to be ≥ 2`、摘要目录 `expected false to be true`；修后树 37/37 全绿）；变异 11 处全被杀（含四处正确性/卫生方向：去掉指纹校验、任意路径都缓存、删除后不清条目、删根不清后代条目）；回归 57 文件全过（session-manager 全目录 / rlm-ledger / agents-view×8 / daemon-catalog×3 / session-artifacts-delete / daemon-mode / daemon-supervisor×18，未跑 4603）；`npm run check`、`check:test-hygiene`、纯净树 `tsgo --noEmit` 均 EXIT=0。
