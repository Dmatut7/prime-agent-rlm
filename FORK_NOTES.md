...
- 修复全部带回归测试（先红后绿验证）| 2026-09-15 凌晨² | 切界面性能（老板亲报）三条里的两条落地：①agents 视图 **40 帧风暴的重建从 40 次降到 1 次**（接 50ms 合并窗 + 只算 dirty 行、别名未变则复用记录）：整目录 pass 6.6→2.6ms（大夹具 28.2→2.0）、带作用域 6.8→1.3（28.6→1.7）②**搜索文本惰性化+跨次复用**：空搜索框 0 次读取（改前每行每遍）、reconcile 21.9→1.1ms；顺带把 heartbeat 无 job 时的每行索引与 session 路径 realpath 结果做 memo（冷 pass 8ms→0.5ms 热）。红绿：基线 6F/1S/3P → 10/10，agents-view 全量 220 + 扩面 585 通过 |
