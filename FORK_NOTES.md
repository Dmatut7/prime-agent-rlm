...
- **验证**：先红后绿（6 个新测试文件 33 条在基线树 `0429806b6` 全红，其中两条是断言级红：并发峰值 `expected 1 to be ≥ 2`、摘要目录 `expected false to be true`；修后树 33/33 全绿）；变异 8 处全被杀（含两处正确性方向：去掉指纹校验、任意路径都缓存）；回归 56 文件 888 条全过（session-manager 全目录 / rlm-ledger / agents-view×8 / daemon-catalog×3 / daemon-mode / daemon-supervisor×18，未跑 4603）；`npm run check`、`check:test-hygiene`、纯净树 `tsgo --noEmit` 均 EXIT=0。
